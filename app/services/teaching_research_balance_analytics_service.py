...
class TeachingResearchBalanceAnalyticsService:
    """Service layer for Teaching vs Research Analytics with clean exception handling."""
    ...
    def get_analytics(self, page: int, page_size: int, filters: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            raise RuntimeError(
                "Teaching vs Research Analytics service is temporarily unavailable. Verify database connection."
            ) from exc
        except Exception as exc:
            raise RuntimeError("An error occurred while processing teaching vs research analytics.") from exc
