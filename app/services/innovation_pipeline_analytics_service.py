...
class InnovationPipelineAnalyticsService:
    """Service layer for Innovation Pipeline Analytics with clean exception handling."""
    ...
    def get_analytics(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            raise RuntimeError(
                "Innovation Pipeline Analytics service is temporarily unavailable. Verify database connection."
            ) from exc
        except Exception as exc:
            raise RuntimeError("An error occurred while processing innovation pipeline analytics.") from exc
